...
const RentalInspectionApp = () => {
  ...
  const goToNextStep = () => {
    ...
    // CRITICAL FIX: Generate rooms BEFORE moving from Step 2 to Step 3
    if (currentStep === 2) {
      console.log('🔄 Step 2 → Step 3: Generating rooms NOW...');
      initializeRooms();
      
      // Small delay to let state update
      setTimeout(() => {
        if (rooms.length === 0) {
          console.warn('⚠️ No rooms generated!');
          alert('⚠️ Please configure at least one room before proceeding.');
          return;
        }
        console.log('✅ Rooms generated successfully! Count:', rooms.length);
